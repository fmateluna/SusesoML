...
def count_licenses_by_diagnosis(df, window_days=30, cods_list=None, entity_col='rut_medico'):
    """
    Cuenta licencias por diagnóstico (letra inicial del código) en una ventana hacia atrás, agrupando por entidad.

    Parámetros:
        df (pd.DataFrame): Debe tener columnas 'fecha_emision', 'cod_diagnostico_principal' y la entidad.
        ...
        entity_col (str): Columna por la cual agrupar (por defecto 'rut_medico').

    Retorna:
        pd.DataFrame: Con columnas adicionales de frecuencia para cada letra de diagnóstico.
    """
    df = df.copy()
